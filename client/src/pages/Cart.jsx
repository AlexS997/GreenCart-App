    ...
    <div className="flex flex-col md:flex-row mt-16">
      <div className="flex-1 max-w-4xl">
        <h1 className="text-3xl font-medium mb-6">
          Shopping Cart <span className="text-sm text-primary px-3">{cartCount()} Items</span>
        </h1>
        ...
        {cartArray.map((product, index) => (
          ...
          >
            ...
            <button onClick={() => removeCartItem(product._id)} className="cursor-pointer mx-auto">
              <img src={assets.remove_icon} alt="removeImg" className="inline-block w-6 h-6"/>
            </button>
          </div>
        ))}

        <button onClick={() => {navigate('/products'); scrollTo(0,0)}} className="group cursor-pointer flex items-center mt-8 gap-2 text-primary font-medium">
          <img src={assets.arrow_right_icon_colored} alt="arrowImg" className="group-hover:-translate-x-1 transition" />
          Continue Shopping
        </button>
      </div>
            ...
            {showAddress && (
              <div className="absolute top-12 py-1 bg-white border border-gray-300 text-sm w-full">
                ...
                {address.map((val, index) => (
                  ...
                  >
                    {val.street}, {val.city}, {val.state}, {val.country}
                  </p>
                )) 
                }
